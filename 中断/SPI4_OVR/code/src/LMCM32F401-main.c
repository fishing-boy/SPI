...
{
	if (SET == SPI_I2S_GetITStatus(SPI4, SPI_I2S_IT_OVR))
	{
		SPI4->CR2 &= ~SPI_CR2_ERRIE;
    SPI4->CR2 &= ~SPI_CR2_RXNEIE;
    printf("open ovr event\r\n");
		SPI_I2S_ClearITPendingBit(SPI4, SPI_I2S_IT_OVR);
	}

  if(SET == SPI_I2S_GetITStatus(SPI4, SPI_I2S_IT_RXNE))
	{
		printf("open rcne event but donot read SPI4->DR\r\n");
	}
}
...
void SPI4_MY_INIT(void)
{
  ...
  SPI4->CR2 |= SPI_CR2_ERRIE|SPI_CR2_RXNEIE;
